...
public class CalibrationUtil {
     ...
     */
    public static void undistort(Mat src, Mat dst, CalibrationData.CalibrationResult params) {
        if (params == null) {
        ...
        Mat distCoeffs = new Mat(DISTORTION_DIM, 1, CvType.CV_64F);
     ...
     * 在MainActivity录制前调用，确保内参与当前分辨率和镜头倍数匹配
     *
     * @param calibData    CalibrationData实例
     * @param currentZoom  当前镜头倍数（如0.6f, 1.0f）
     * @param recordWidth  实际录制宽度
     * @param recordHeight 实际录制高度
     */
    public static CalibrationData.CalibrationResult getParamsForRecording(
            CalibrationData calibData, float currentZoom, int recordWidth, int recordHeight) {

        CalibrationData.CalibrationResult result =
                calibData.getCalibrationForUse(currentZoom, recordWidth, recordHeight);

        if (result == null) {
            Log.e(TAG, String.format("未找到 %.1fx / %dx%d 的标定参数，请先标定", currentZoom, recordWidth, recordHeight));
        } else {
            Log.i(TAG, String.format("使用 %.1fx / %dx%d 标定参数，fx=%.2f, fy=%.2f",
                    result.zoomLevel, result.imageWidth, result.imageHeight, result.fx, result.fy));
        }
        return result;
    }
}